...
<%
	...
	customRespBean.setRespType("text@unknown");
	...
	if (result != -1) {
		request.getSession()
				.setAttribute("addUnknownRespStatus", "添加成功");
		response.sendRedirect("/WXPortal/unknownResp.jsp?wxaccountid="
				+ wxaccountid);
	} else {
		request.getSession()
				.setAttribute("addUnknownRespStatus", "添加失败");
		response.sendRedirect("/WXPortal/unknownResp.jsp?wxaccountid="
				+ wxaccountid);
	}
%>
