...
public class CustomRespDBService {
	...
	/**
	 * 判断微信账号是否存在
	 * @param column:取值范围：name，orgid，wxnumber，token
	 * @param value
	 * @return
	 */
	@SuppressWarnings("unchecked")
	public boolean isCustomRespExist(String ColumnName,String ColumnValue,String  wxAccountId){
		boolean bFlag = true;
		String whereCause = " where "+ColumnName+" = '"+ColumnValue+"' and wxAccountId = "+wxAccountId;
		ArrayList<CustomRespBean> list = new ArrayList<CustomRespBean>();
		list = (ArrayList<CustomRespBean>)dao.queryByCondition("CustomRespBean", whereCause, 0, 0);
		if(list.size()>0) bFlag = false;
		return bFlag;
	}
	 ...
	 * @return
	 */
	public boolean delCustomRespValues(String CustomRespBean,String[] columns,String[] values){
	 ...
	 * @param columns：属性的数组
	 * @param values：属性对应的值
	 * @param whereCause：where条件
	 */
	public void updateCustomRespByValue(String CustomRespBean,String[] columns,String[] values,String whereCause){
	 ...
	 * @param wxAccountId:微信id
	 * @param type:取值范围为：01：text 02：music 
	 *             03：image 04：link 05:  news 06:  cmd 或者 ""：表示所有的
	 * @param page: 为0 不分页 大于零分页查询
	 * @return
	 */
	public ArrayList<CustomRespBean> queryCustomResps(int wxAccountId,String type,int page,int pageSize){
		String whereCause = "";
		ArrayList<CustomRespBean> list = new ArrayList<CustomRespBean>();
		if("".equals(type)){
			whereCause = " wxAccountId = "+wxAccountId;
		}else{
			whereCause = " where type = '"+type+"' and wxAccountId = "+wxAccountId;
		}
